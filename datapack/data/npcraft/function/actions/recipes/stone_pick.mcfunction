...
function npcraft:inventory/remove {kind:"minecraft:cobblestone",count:3}
...
execute unless score #inv_ok np.tmp matches 1 run return run function npcraft:actions/result {state:"blocked",reason:"missing_ingredients"}
data modify storage npcraft:inv input set value {item:{id:"minecraft:stone_pickaxe",count:1,components:{"minecraft:damage":0}},max:1}
