...
def run(args):
    ...
    with tempfile.TemporaryDirectory(prefix='npcraft-agent-gui-') as tmp:
        ...
        try:
            ...
            check('backpack_preserves_stack_and_components', f'if data entity {BOT} data.inventory.slots[0].item{{id:"minecraft:diamond",count:20,components:{{"minecraft:custom_name":{{text:"Backpack QA"}}}}}}')
